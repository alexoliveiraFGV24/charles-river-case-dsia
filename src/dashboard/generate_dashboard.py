...
from src.database import engine, DadosCotacao, IndicadoresFundamentalistas, Ativo
...
if tipo_dados in ("Cotação", "Ambos"):
    ...
    if df_cot.empty:
        ...
        # Seletor de métricas de cotação
        ...
        for metric in sel_cot_metrics:
            fig = line_chart(df_cot, metric, COTACAO_COLS[metric], sel_tickers)
...
if tipo_dados in ("Indicadores Fundamentalistas", "Ambos"):
    ...
    if df_ind.empty:
        ...
        # Agrupa em pares de colunas para melhor uso do espaço
        for i in range(0, len(sel_ind_metrics), 2):
            pair = sel_ind_metrics[i:i+2]
            cols = st.columns(len(pair))
            for j, metric in enumerate(pair):
                fig = line_chart(df_ind, metric, INDICADOR_COLS[metric], sel_tickers)
...
# ══════════════════════════════════════════════════════════════════════════════
#  SEÇÃO 5 – COMPARATIVO ENTRE TICKERS (último valor)
# ══════════════════════════════════════════════════════════════════════════════
...
if len(sel_tickers) > 1:
    ...
    with tab_cot:
        if not df_cot.empty:
            ...
            metric_cot = st.selectbox("Métrica", list(COTACAO_COLS.keys()),
                                       format_func=lambda x: COTACAO_COLS[x], key="cmp_cot")
            fig = px.bar(latest_cot, x="Ticker", y=metric_cot,
                         color="Ticker", title=COTACAO_COLS[metric_cot],
    ...
    with tab_ind:
        if not df_ind.empty:
            ...
            metric_ind = st.selectbox("Indicador", list(INDICADOR_COLS.keys()),
                                       format_func=lambda x: INDICADOR_COLS[x], key="cmp_ind")
            fig = px.bar(latest_ind, x="Ticker", y=metric_ind,
                         color="Ticker", title=INDICADOR_COLS[metric_ind],
